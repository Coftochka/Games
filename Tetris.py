...
class part:
	...
	def check_free(self, mod1, mod2):
		...
		if 0 <= b < 20 and 0 <= a < 10:
			#pq += f'pg.draw.circle(scr, (0, 255, 0), (({a} + 0.5) * res, ({b} + 0.5) * res), 7)\n'
			global arr
			return arr[b, a].get()
		return 1
	def delete(self):
...
class figure:
	...
	def rotate(self, right):
		...
		for i in range(4):
			...
			a = c[0] - a
			b = c[1] - b
			ans += self.parts[i].check_free(a, b)
		if ans == 0:
			for i in self.parts:
				i.delete()
			for i in range(4):
...
class f1(figure):#
	def __init__(self):
		super().__init__(4, 1, [(-1, 0), (0, 0), (1, 0), (2, 0)], (255, 255, 255))
class f2(figure):#
	def __init__(self):
...
timer = pg.time.Clock()
arr = np.array([[filler() for _ in range(10)] for i in range(20)])
cond = True
without_stick = 0
